...
/// Signature : 665739977b2b3a2a994085d0bdfc2280666224ab
/// ***************************************************************************
/// </summary>
  ...
  InputQuery('Key size', ['How many values do you want for this key ?'],
    ...
    begin
      ...
      begin
        ...
        FKeySize := i;
        Key := TOlfCryptDecrypt.GenShiftKey(FKeySize);
...
begin
  url_Open_In_Browser
    ('https://github.com/DeveloppeurPascal/librairies/tree/master/samples/RTL.CryptDecrypt/ShiftCryptDecryptSample');
end;
...
begin
  ...
  if length(tab) <> FKeySize then
    raise exception.Create('Wrong number of numbers in the list. (' +
      length(tab).ToString + ' <> ' + FKeySize.ToString + ')');

  Key := TList<integer>.Create;
  try
...
procedure TfrmMain.FillPascalCode(const Key: TList<integer>);
var
  i: integer;
  s: string;
begin
  ...
  begin
    ...
    mmoPascal.Lines.add('var Key:array of integer;');
...
begin
  ...
  OlfAboutDialog1.Description.Text :=
    'Shift keys Generator creates a randomly generated serie of numbers between -7 and 7 as an array depending on the language you use.'
    ...
    + slinebreak + slinebreak +
    'To find out more, visit https://shiftkeysgenerator.olfsoftware.fr';
end;
...
{$ENDIF}
TParams.InitDefaultFileNameV2('OlfSoftware', 'ShiftKeysGen', false);
{$IF Defined(RELEASE)}
